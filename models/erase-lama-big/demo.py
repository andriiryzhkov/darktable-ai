...
def run_inference(model_path, image_path, output_path, mask_str):
    """Run inpainting inference with a rectangular mask."""
    os.makedirs(os.path.dirname(output_path) or ".", exist_ok=True)

    x1, y1, x2, y2 = [float(v) for v in mask_str.split(",")]
...
def main():
    ...
    run_inference(args.model, args.image, args.output, args.mask)


def demo(model, image, output, **kwargs):
    """Entry point for programmatic demo."""
    mask = kwargs.get("mask", "0.25,0.25,0.75,0.75")
    run_inference(model, image, output, mask)
