# Export SAM 2.1 HQ image encoder and decoder (prompt encoder + mask decoder)
# to separate ONNX files:  encoder.onnx  and  decoder.onnx.
#
# Based on the SAM 2.1 export script with adaptations for the HQ variant
# from https://github.com/SysCV/sam-hq (sam-hq2).
#
# MaskDecoderHQ.predict_masks() returns 5 masks instead of 4:
#   index 0: single-mask token
#   index 1-3: multi-mask tokens (3 SAM masks)
#   index 4: HQ token (high-quality refinement mask)
#
# The decoder ONNX model outputs 4 masks (3 SAM + 1 HQ), 3 IoU predictions
# (for the SAM masks only), and 4 low-res masks for iterative refinement.
...
parser = argparse.ArgumentParser(
    description="Export SAM 2.1 HQ encoder and decoder to separate ONNX files."
)
parser.add_argument(
    "--model-cfg", type=str, required=True,
    help="Hydra config file (e.g. configs/sam2.1/sam2.1_hq_hiera_l.yaml).",
)
parser.add_argument(
    "--checkpoint", type=str, required=True,
    help="Path to the SAM 2.1 HQ model checkpoint (.pt).",
)
parser.add_argument(
...
class SAM2EncoderOnnxModel(nn.Module):
    """Wraps the SAM 2.1 HQ image encoder for ONNX export.

    Identical to the standard SAM 2.1 encoder wrapper — SAM2HQBase has
    the same image_encoder, no_mem_embed, and conv_s0/conv_s1 interface.
...
class SAM2HQDecoderOnnxModel(nn.Module):
    """Wraps the SAM 2.1 HQ prompt encoder + mask decoder for ONNX export.

    MaskDecoderHQ.predict_masks() returns 5 masks (single + 3 multi + 1 HQ)
    and 4 IoU predictions (single + 3 multi, no IoU for HQ token).

    In multimask mode, outputs:
        masks:           (B, 4, 1024, 1024)  — 3 SAM multi-masks + 1 HQ mask
        iou_predictions: (B, 3)              — IoU scores for the 3 SAM masks
        low_res_masks:   (B, 4, 256, 256)    — low-res versions for refinement

    The last channel (index 3) in masks/low_res_masks is the HQ mask.
    """
    ...
    def forward(
        ...
        # MaskDecoderHQ returns 5 masks: single(0), multi(1,2,3), hq(4)
        # iou_predictions has 4 values: single(0), multi(1,2,3)
        all_masks, iou_predictions, _, _ = self.mask_decoder.predict_masks(
        ...
        masks = F.interpolate(
            all_masks, size=(1024, 1024), mode="bilinear", align_corners=False
        )

        if self.multimask_output:
            # SAM multi-masks (indices 1,2,3) + HQ mask (index 4)
            sam_masks = masks[:, 1:4, :, :]
            hq_mask = masks[:, 4:5, :, :]
            masks = torch.cat([sam_masks, hq_mask], dim=1)

            iou_predictions = iou_predictions[:, 1:4]

            sam_low = all_masks[:, 1:4, :, :]
            hq_low = all_masks[:, 4:5, :, :]
            low_res_masks = torch.cat([sam_low, hq_low], dim=1)
        else:
...
def run_decoder_export(
    ...
    decoder = SAM2HQDecoderOnnxModel(sam_model, multimask_output=multimask_output)
    decoder.eval()
